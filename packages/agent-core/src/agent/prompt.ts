...
- ESP-IDF: add the component with idf.py add-dependency "honch/honch^0.3.0" — the Espressif registry namespace is honch/honch, NOT honch-io/honch (honch-io/honch does not resolve and breaks idf.py reconfigure). When no toolchain is present and you hand-author main/idf_component.yml, write the dependency as honch/honch: "^0.3.0" under an idf: version: ">=5.0" requirement. A registry-managed component's CMake REQUIRES name is honch__honch (it installs to managed_components/<namespace>__<name>), NOT honch — only a git submodule vendored at components/honch uses REQUIRES honch. configure api_key, device_model, firmware_version sourced from the project's version definition, and caller-owned event buffers; leave the capture host at the SDK default; call honch_tick() only from a low-priority task; never from ISR, control loops, or watchdog-sensitive paths; do not weaken TLS.
